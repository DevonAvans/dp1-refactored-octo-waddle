...
namespace test::game_state
{
	...
	public:
		...
		TEST_METHOD(change_value_helper_game_state)
		{
			...
			const bool contains_value3 = std::ranges::find(candidates, candidate1) != candidates.end();
			const bool contains_value6 = std::ranges::find(candidates, candidate2) != candidates.end();
			...
			Assert::IsTrue(contains_value6, L"Candidates should contain the value 6");
		}
